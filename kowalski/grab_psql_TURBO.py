...
#                    print(data_bytes)
                    ...
                    if keys[i]=='candid':
                        alert[keys[i]]=int(result[0])
#                        print(result[i])
                    elif keys[i]=='objectId':
                        alert[keys[i]]=str(result[-1])
#            else:
#            print(len(keys),len(
#                print('Length of keys does not match length of items. Dictionary cant be created.')
            #most of the things below here need to be updated with accurate stuff.  doing temporary random things now to get thigns to work
            candic={}
            for i in keys2:
                if i=='ra':
                    candic['ra']=result[3]
...
#                    print(Time(result[-2]).jd)
                    candic['jd']=Time(result[-2]).jd
                elif i=='fid':#field id could be updated when we have different cameras/telescopes pointing to different parts of the sky.
                    candic['fid']=1
                elif i=='programid':#surveymode or other
                    candic['programid']=1
                elif i=='candid':
                    candic['candid']=int(result[0])
                elif i=='isdiffpos':#is the candidate a positive or negative after subtraction. can try to get this from zogy
                    candic['isdiffpos']=1
                elif i=='magpsf':#magnitude from PSF-fit photometry [mag]. need to do zero point calculation
                    candic['magpsf']=1.0
                elif i=='sigmapsf':#1-sigma uncertainty in magpsf [mag]
                    candic['sigmapsf']=0.1
                elif i=='psra1':#Right Ascension of nearest source in reference image PSF-catalog; J2000 [deg]. need to do cross matching 
                    candic['psra1']=1
                elif i=='psdec1':#Declination of nearest source in reference image PSF-catalog; J2000 [deg]. need to do cross matching
                    candic['psdec1']=1
                elif i=='nmtchps':#Number of source matches from PS1 catalog falling within 30 arcsec. need to do cross matching
                    candic['nmtchps']=1
                elif i=='nmtchtm':#Number of source matches from 2MASS catalog falling within 30 arcsec. need to do cross matching
                    candic['nmtchtm']=1
