...
async def zuds_alert_get_cutout(request):
    ...
    try:
        ...
        if file_format == 'png':
            ...
            img_corrected = np.arcsinh(img - np.median(img))
            vmax = np.percentile(img_corrected, 98)
            vmin = np.percentile(img_corrected, 2)
            plt.imshow(img_corrected, cmap='bone', origin='lower', vmax=vmax, vmin=vmin)
            plt.savefig(buff, dpi=42)
