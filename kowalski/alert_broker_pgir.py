...
def make_photometry(alert: dict, jd_start: float = None):
    ...
    :param alert: PGIR alert packet/dict
    :param jd_start:
    :return:
    """
    ...
    df_light_curve["pgir_filter"] = "2massj"
    ...
    # 23.0 (results in flux in uJy for 2MASS system)
    df_light_curve["flux"] = coeff * 10 ** (-0.4 * (df_light_curve["magpsf"] - 23.0))
...
def make_thumbnail(alert, ttype: str, pgirtype: str):
    """
    Convert lossless FITS cutouts from PGIR alerts into PNGs

    :param alert: PGIR alert packet/dict
    :param ttype: <new|ref|sub>
    :param pgirtype: <Science|Template|Difference>
    :return:
    """
    alert = deepcopy(alert)

    cutout_data = alert[f"cutout{pgirtype}"]["stampData"]
    ...
    norm = ImageNormalize(
        img, stretch=LinearStretch() if pgirtype == "Difference" else LogStretch()
    )
    img_norm = norm(img)
...
def make_triplet(alert, to_tpu: bool = False):
    """
    Make an L2-normalized cutout triplet out of a PGIR alert
...
def alert_filter__ml(alert, ml_models: dict = None) -> dict:
    """Execute ML models on a PGIR alert
    ...
    # Keeping this commented for now until we have a ML model to run on kowalski
    """
...
# cone search radius:
cone_search_radius = float(config["database"]["xmatch_pgir"]["cone_search_radius"])
# convert to rad:
if config["database"]["xmatch_pgir"]["cone_search_unit"] == "arcsec":
    cone_search_radius *= np.pi / 180.0 / 3600.0
elif config["database"]["xmatch_pgir"]["cone_search_unit"] == "arcmin":
    cone_search_radius *= np.pi / 180.0 / 60.0
elif config["database"]["xmatch_pgir"]["cone_search_unit"] == "deg":
    cone_search_radius *= np.pi / 180.0
elif config["database"]["xmatch_pgir"]["cone_search_unit"] == "rad":
...
def alert_filter__xmatch(database, alert) -> dict:
    ...
    try:
        ...
        """ catalogs """
        for catalog in config["database"]["xmatch_pgir"]["catalogs"]:
            catalog_filter = config["database"]["xmatch_pgir"]["catalogs"][catalog][
                "filter"
            ]
            catalog_projection = config["database"]["xmatch_pgir"]["catalogs"][catalog][
                "projection"
            ]
...
def process_alert(record, topic):
    ...
    #       move cutouts to PGIR_alerts_cutouts? reduce the main db size for performance
...
class AlertConsumer:
    ...
    def __init__(self, topic, dask_client, **kwargs):
        ...
        self.collection_alerts = config["database"]["collections"]["alerts_pgir"]
...
class AlertWorker:
    ...
    def __init__(self, **kwargs):
        ...
        self.collection_alerts = self.config["database"]["collections"]["alerts_pgir"]
        ...
        for model in config["ml_models_pgir"]:
            try:
                model_version = config["ml_models_pgir"][model]["version"]
                # todo: allow other formats such as SavedModel
                model_filepath = os.path.join(
                    config["path"]["ml_models_pgir"], f"{model}.{model_version}.h5"
                )
        ...
        # get PGIR instrument id
        self.instrument_id = 1
        with timer("Getting PGIR instrument_id from SkyPortal", self.verbose > 1):
            response = self.api_skyportal("GET", "/api/instrument", {"name": "PGIR"})
        if response.json()["status"] == "success" and len(response.json()["data"]) > 0:
            self.instrument_id = response.json()["data"][0]["id"]
            log(f"Got PGIR instrument_id from SkyPortal: {self.instrument_id}")
        else:
            log("Failed to get PGIR instrument_id from SkyPortal")
            raise ValueError("Failed to get PGIR instrument_id from SkyPortal")

        # get PGIR alert stream ids to program ids mapping
        self.pgir_program_id_to_stream_id = dict()
        with timer("Getting PGIR alert stream ids from SkyPortal", self.verbose > 1):
            response = self.api_skyportal("GET", "/api/streams")
        if response.json()["status"] == "success" and len(response.json()["data"]) > 0:
            for stream in response.json()["data"]:
                if stream.get("name") == "PGIR All":
                    self.pgir_program_id_to_stream_id[0] = stream["id"]
            if len(self.pgir_program_id_to_stream_id) != 1:
                log("Failed to map PGIR alert stream ids from SkyPortal to program ids")
                raise ValueError(
                    "Failed to map PGIR alert stream ids from SkyPortal to program ids"
                )
            log(
                f"Got PGIR program id to SP stream id mapping: {self.pgir_program_id_to_stream_id}"
            )
        else:
        ...
        return list(
            self.mongo.db[config["database"]["collections"]["filters_pgir"]].aggregate(
                [
                    {
                        "$match": {
                            "catalog": config["database"]["collections"]["alerts_pgir"],
                            "active": True,
                        }
                    },
                    {
    ...
    def alert_mongify(alert: Mapping):
        """
        Prepare a raw PGIR alert for ingestion into MongoDB:
    ...
    def alert_post_candidate(self, alert: Mapping, filter_ids: Sequence):
        """
        Post a PGIR alert as a candidate for filters on SkyPortal
        :param alert:
        :param filter_ids:
        :return:
        """
    ...
    def alert_post_source(self, alert: Mapping, group_ids: Sequence):
        """
        Save a PGIR alert as a source to groups on SkyPortal
    ...
    def alert_post_thumbnails(self, alert: Mapping):
        """
        Post PGIR alert thumbnails to SkyPortal

        :param alert:
        :return:
        """
        for ttype, pgirtype in [
            ("new", "Science"),
            ("ref", "Template"),
            ("sub", "Difference"),
        ]:
            with timer(
                f"Making {pgirtype} thumbnail for {alert['objectId']} {alert['candid']}",
                self.verbose > 1,
            ):
                thumb = make_thumbnail(alert, ttype, pgirtype)

            with timer(
                f"Posting {pgirtype} thumbnail for {alert['objectId']} {alert['candid']} to SkyPortal",
                self.verbose > 1,
            ):
            ...
            if response.json()["status"] == "success":
                log(
                    f"Posted {alert['objectId']} {alert['candid']} {pgirtype} cutout to SkyPortal"
                )
            else:
                log(
                    f"Failed to post {alert['objectId']} {alert['candid']} {pgirtype} cutout to SkyPortal"
                )
                log(response.json())
    ...
    def alert_put_photometry(self, alert):
        ...
        with timer(
            ...
            df_photometry["stream_id"] = df_photometry["programid"].apply(
                lambda programid: self.pgir_program_id_to_stream_id[programid]
            )
        ...
        for stream_id in set(df_photometry.stream_id.unique()):
            ...
            photometry = {
                ...
                "filter": df_photometry.loc[stream_id_mask, "pgir_filter"].tolist(),
    ...
    def alert_sentinel_skyportal(self, alert, prv_candidates, passed_filters):
        ...
        :param alert: PGIR_alert with a stripped-off prv_candidates section
...
def watchdog(obs_date: str = None, test: bool = False):
    ...
    while True:
        ...
        try:
            ...
            # as of 20210525, the naming convention is pgir_%Y%m%d
