...
class KafkaStream:
    ...
    def start(self):
        ...
        path_logs = pathlib.Path(self.config["path"]["logs"])
        ...
        cmd_zookeeper = [
            os.path.join(
                self.config["path"]["kafka"], "bin", "zookeeper-server-start.sh"
            ),
            "-daemon",
            os.path.join(
                self.config["path"]["kafka"], "config", "zookeeper.properties"
            ),
        ]
        ...
        cmd_kafka_server = [
            os.path.join(self.config["path"]["kafka"], "bin", "kafka-server-start.sh"),
            "-daemon",
            os.path.join(self.config["path"]["kafka"], "config", "server.properties"),
        ]

        with open(
            os.path.join(self.config["path"]["logs"], "kafka_server.stdout"), "w"
        ):
        ...
        cmd_topics = [
            os.path.join(self.config["path"]["kafka"], "bin", "kafka-topics.sh"),
            "--bootstrap-server",
        ...
        if self.topic not in topics:
            ...
            with open(
                os.path.join(self.config["path"]["logs"], "create_topic.stdout"), "w"
    ...
    def stop(self):
        ...
        cmd_kafka_server_stop = [
            os.path.join(self.config["path"]["kafka"], "bin", "kafka-server-stop.sh"),
            os.path.join(self.config["path"]["kafka"], "config", "server.properties"),
        ]
        ...
        cmd_zookeeper_stop = [
            os.path.join(
                self.config["path"]["kafka"], "bin", "zookeeper-server-stop.sh"
            ),
            os.path.join(
                self.config["path"]["kafka"], "config", "zookeeper.properties"
            ),
        ...
        with open(
            os.path.join(self.config["path"]["logs"], "zookeeper.stdout"), "w"
        ) as stdout_zookeeper:
            # p_zookeeper_stop = \
            subprocess.run(
